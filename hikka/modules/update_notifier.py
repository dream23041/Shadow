...
class UpdateNotifier(loader.Module):
    ...
    async def client_ready(self):
        ...
        self._markup = lambda: self.inline.generate_markup(
            [
                {"text": self.strings("update"), "data": "shadow/update"},
                {"text": self.strings("ignore"), "data": "shadow/ignore_upd"},
            ]
        )
        ...
        if self._pending not in {utils.get_git_hash(), self._notified}:
            m = await self.inline.bot.send_animation(
                ...
                caption=self.strings("update_required").format(
                    ...
                    '<a href="https://github.com/Viciu3/Shadow/compare/{}...{}">{}</a>'.format(
    ...
    async def update(self, call: InlineCall):
        ...
        if call.data not in {"shadow/update", "shadow/ignore_upd"}:
            return

        if call.data == "shadow/ignore_upd":
    ...
    async def changelog(self, message: Message):
        ...
        if (await self._client.get_me()).premium:
            changelog.replace('👻 Shadow', '<emoji document_id=5192765204898783881>🌘</emoji><emoji document_id=5195311729663286630>🌘</emoji><emoji document_id=5195045669324201904>🌘</emoji>')
